...
class LeCartCustom(LeBasecart):
	...
	def get_categories_main_export(self):
		...
		''' Main information in table 'categories' '''
	...
	def get_categories_ext_export(self, categories):
		''' Additional information in table 'categories_description' '''
		...
		categories_ext_queries = {
			'categories_description': {  # tablename_field
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_categories_description WHERE categories_id IN " + self.list_to_in_condition(category_ids)
			},
			'languages': {  # tablename
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_languages as lg INNER JOIN  _DBPRF_categories_description as cd"
				         + " ON  lg.languages_id = cd.language_id WHERE cd.categories_id IN " + self.list_to_in_condition(map(int, category_ids))
			},
		}
		categories_ext = self.select_multiple_data_connector(categories_ext_queries, 'categories')  # log name
	...
	def convert_category_export(self, category, categories_ext):
		...
		parent['id'] = 0
		if category['parent_id']:  # exists in categories table
	...
	def get_products_ext_export(self, products):
		...
		# image_ids=duplicate_field_value_from_list(product_ext['data']['products_image'], 'image_id')
	...
	def convert_product_export(self, product, products_ext):
		...
		# product_data['construct_product']['path'] = 'https://images.unsplash.com/photo-1588779180563-d7599d127ffe?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=800&q=80'
	...
	def get_customers_ext_export(self, customers):
		...
		customer_ext_queries = {
			'address_book': {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_address_book WHERE customers_id IN " + self.list_to_in_condition(customer_ids)
			}
		}
	...
	def convert_order_export(self, order, orders_ext):
		...
		# customer_country = get_row_from_list_by_field(orders_ext['data']['countries'], 'countries_id', order['bill_cntry'])
		# if customer_country:
	...
	def get_category_parent(self, category_id):
		...
		categories = self.select_data_connector(query, 'categories')  # show log
		if not categories or categories['result'] != 'success':
